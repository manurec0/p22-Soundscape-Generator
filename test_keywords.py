...
# query = input("What type of sound do you want to search: ")
# number = int(input("Number of sounds: "))
# durationmin = (input("Minimum duration: "))
# durationmax = (input("Maximum duration: "))
# tag = input("Tag of the sound: ")

# if durationmin == '' and durationmax == '':
#     duration = None
# elif durationmin == '':
#     duration = (0.0, float(durationmax))
# elif durationmax == '':
#     duration = (float(durationmin), MAX_DURATION)
# else:
#     duration = (float(durationmin), float(durationmax))

# if tag == '':
#     tag = None

# download_sounds_freesound(queryText=query, tag=tag, duration=duration, API_Key="xTdc6oqv5FLJQCmdcD8kRKnzuYnScA6vrkXBs36v", outputDir= os.getcwd(), topNResults=number,featureExt='.json')
rake_nltk_var = Rake()
text_en = "People chatting when walking by the city"
text_es = "Gente paseando en la ciudad"
rake_nltk_var.extract_keywords_from_text(text_en)
keyword_extracted = rake_nltk_var.get_ranked_phrases()
print(keyword_extracted)
print(len(keyword_extracted))
for i in range(len(keyword_extracted)):
    download_sounds_freesound(queryText = keyword_extracted[i], tag = None, duration = None, API_Key = "wwqd0QQRY6P2enLsu1HK1PyADEojnL5UPXEIsjQe", outputDir = os.getcwd(), topNResults = 1, featureExt = '.json')
    
#francisco lopez
#Chris cutler
#pyo
#pnl (processament de llenguatge natural)
#gpt3
